        ...
        }else
        {
            foreach (var vector4 in list)
            {
                ...
                if (x >= 0 && y >= 0 && x < width && y < height)
                {
                    ...
                    uv[index * 4 + 0] = PixelsToUV(idX * 32 + (idX * 2), idY * 32 + hs);
                    uv[index * 4 + 1] = PixelsToUV(idX * 32 + ws + (idX * 2), idY * 32 + hs);
                    uv[index * 4 + 2] = PixelsToUV(idX * 32 + (idX * 2), idY * 32);
                    uv[index * 4 + 3] = PixelsToUV(idX * 32 + ws + (idX * 2), idY * 32);
                }
            }
        }
        mesh.vertices = vertices;
